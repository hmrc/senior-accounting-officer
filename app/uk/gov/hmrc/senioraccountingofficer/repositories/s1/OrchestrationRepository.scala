...
final case class SubmissionOrchestration(correlationId: String, jobType: String)
...
class OrchestrationRepository @Inject()(
    ...
    extends WorkItemRepository[SubmissionOrchestration](
      collectionName = "s1-orchestration",
      mongoComponent = mongoComponent,
...
//    Duration(configuration.get[Long]("queue.retryAfter"), TimeUnit.SECONDS).toJava
