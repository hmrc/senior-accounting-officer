...
object SubmitCertificateHelper {
  def mock(subscriptionId: String, status: Int, body: Option[String]): StubMapping = {
    stubFor(
      post(urlEqualTo(s"/dapm/subscriptions/$subscriptionId/certificates"))
        .willReturn(
          body match {
            case Some(body) =>
              aResponse()
  ...
  def verifyCalled(subscriptionId: String, body: Option[String], times: Int): Unit = {
    val postRequest = postRequestedFor(urlEqualTo(s"/dapm/subscriptions/$subscriptionId/certificates"))
    verify(
      times,
